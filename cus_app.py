...
# --------------------------------------------------
def _extract_aspect_text(text: str, aspect: str) -> str:
    """
    Extracts only the clause(s) from the review that are relevant to the given aspect.
    This prevents positive words from one part of a review (e.g. 'I love this shoe')
    from contaminating the BERT prediction for a different aspect (e.g. service).

    Strategy:
    1. Split text into clauses/sentences by common delimiters and conjunctions.
    2. Score each clause by how many aspect keywords it contains.
    3. Return the highest-scoring clause(s), or the full text as fallback.
    """
    import re

    ASPECT_EXTRACT_KEYWORDS = {
        "product":  ["product", "item", "quality", "material", "broken", "defective",
                     "packaging", "box", "damaged", "arrived", "delivery", "works",
                     "battery", "screen", "size", "color", "taste", "food", "meal",
                     "shoe", "shoes", "sneaker", "cloth", "clothing", "phone", "laptop",
                     "fabric", "soft", "hard", "fit", "design", "smell", "build",
                     "feature", "performance", "durable", "fragile", "cracked", "torn",
                     "scratched", "faulty", "defect", "ordered", "order", "shipped"],
        "service":  ["service", "staff", "employee", "waiter", "waitress", "cashier",
                     "manager", "team", "host", "behavior", "behaviour", "attitude",
                     "rude", "helpful", "friendly", "polite", "ignored", "waiting",
                     "treatment", "response", "support", "assist", "care", "attention",
                     "professional", "representative", "receptionist", "greeted", "server"],
        "price":    ["price", "cost", "expensive", "cheap", "overpriced", "value",
                     "worth", "afford", "discount", "deal", "money", "fee", "charge",
                     "bill", "pay", "paid", "rate", "budget", "pricey", "refund",
                     "ripoff", "rip-off", "rip off", "overcharged"],
        "ambience": ["ambience", "ambiance", "atmosphere", "environment", "vibe",
                     "noise", "noisy", "dirty", "clean", "smell", "lighting", "decor",
                     "crowd", "spacious", "cramped", "seating", "location", "parking",
                     "temperature", "hygiene", "washroom", "bathroom", "interior",
                     "music", "place", "store", "shop", "restaurant"],
    }

    keywords = ASPECT_EXTRACT_KEYWORDS.get(aspect.lower(), [])

    # Split into clauses on common delimiters + conjunctions
    clauses = re.split(
        r'[.!?;]|\band\b|\bbut\b|\bhowever\b|\bthough\b|\balthough\b|\bwhile\b'
        r'|\byet\b|\bwhereas\b|\bapart from\b|\bbesides\b',
        text, flags=re.IGNORECASE
    )
    clauses = [c.strip() for c in clauses if c.strip()]

    # Score each clause by aspect keyword hits
    scored = []
    ...
    # If best clause has at least one keyword match, use top matching clauses
    if scored and scored[0][0] > 0:
        best_clauses = [c for hits, c in scored if hits == scored[0][0]]
        return " ".join(best_clauses)
...
def analyze_aspect(text, aspect):
    ...
    # ── ASPECT-FOCUSED EXTRACTION ────────────────────────────────────────
    # Extract only the clause(s) relevant to this aspect before sending to BERT.
    # This prevents "I love this shoe" from making service prediction Positive
    # when the service part says "staff behavior is not up to the level".
    focused_text = _extract_aspect_text(text, aspect)

    negative_keywords = {
        "product": [
            "bad", "cold", "stale", "broken", "defective", "poor", "cheap",
            "tasteless", "raw", "burnt", "damaged", "late", "delay", "faulty",
            "not working", "stopped", "malfunction", "issue", "cracked", "torn",
            "scratched", "fragile", "flimsy", "inferior", "horrible", "terrible",
            "awful", "worst", "useless", "disappointed", "disappointing",
            "waste", "not good", "not great", "not worth", "didn't work",
            "doesn't work", "stop working", "broke", "falling apart",
        ],
        "service": [
            "rude", "slow", "ignored", "late", "waiting", "unhelpful",
            "unprofessional", "disrespectful", "arrogant", "impolite",
            "unkind", "mean", "harsh", "hostile", "attitude", "careless",
            "negligent", "indifferent", "inefficient", "not up to", "below",
            "poor service", "bad service", "terrible service", "worst service",
            "not helpful", "not friendly", "not polite", "not good",
            "not at all", "no care", "no attention", "no response",
            "not satisfied", "unacceptable", "pathetic", "disappointing",
        ],
        "price": [
            "expensive", "costly", "high", "overpriced", "rip-off", "ripoff",
            "rip off", "too much", "not worth", "pricey", "steep", "waste of money",
            "outrageous", "robbery", "overcharged", "hidden fee",
        ],
        "ambience": [
            "dirty", "noisy", "smell", "loud", "crowded", "messy", "filthy",
            "unhygienic", "dusty", "smelly", "stained", "pest", "cockroach",
            "dark", "gloomy", "cramped", "uncomfortable", "stuffy", "freezing",
        ],
    }

    neutral_words = ["okay", "average", "fine", "decent", "normal", "acceptable"]

    # -------- 1️⃣ MODEL PREDICTION on FOCUSED TEXT --------
    inputs = tokenizer(
        focused_text,
        text_pair=aspect,
        return_tensors="pt",
    ...
    # -------- 2️⃣ NEGATION KEYWORD OVERRIDE --------
    # If BERT predicts Positive/Neutral BUT clear negative keywords for this
    # aspect exist in the focused text, override to Negative.
    focused_lower = focused_text.lower()
    neg_hits = [kw for kw in negative_keywords.get(aspect, []) if kw in focused_lower]
    if neg_hits and sentiment in ("Positive", "Neutral"):
        sentiment = "Negative"
        confidence = max(confidence, 0.72)   # report reasonable confidence
...
def get_relevant_aspects(text):
    ...
    try:
        ...
        # If a signal keyword is present in the text → low threshold (0.30)
        #   because the keyword is strong semantic evidence by itself.
        # If NO keyword is present → high threshold (0.60)
        #   because we must rely entirely on zero-shot model confidence.
        #
        # This fixes BOTH problems:
        #   ✅ "staff behavior" → "staff" keyword found → service included (even at 0.38 score)
        #   ✅ Pure product review → no service keyword → service blocked (needs >0.60 score)

        aspect_keywords = {

            # ── PRODUCT ─────────────────────────────────────────────────────
            # Physical goods, quality, durability, appearance, specs, delivery
            "product": [
                # General product terms
                "product", "item", "goods", "merchandise", "thing", "stuff",
                "article", "unit", "piece", "object", "purchase",
                # Quality
                "quality", "build", "material", "construction", "craft",
                "durable", "durability", "sturdy", "fragile", "flimsy", "solid",
                "premium", "cheap", "inferior", "excellent", "terrible", "awful",
                "amazing", "fantastic", "horrible", "poor", "mediocre",
                # Physical state
                "broken", "damaged", "defective", "faulty", "cracked", "torn",
                "scratched", "dented", "bent", "warped", "worn", "frayed",
                "leaking", "missing", "incomplete", "wrong",
                # Design / Appearance
                "design", "look", "appearance", "color", "colour", "size",
                "shape", "style", "finish", "texture", "weight", "fit",
                "pattern", "print", "logo", "brand",
                # Specific product types
                "shoe", "shoes", "sneaker", "boot", "sandal", "slipper",
                "cloth", "clothing", "clothes", "shirt", "dress", "pants",
                "jacket", "bag", "watch", "phone", "laptop", "computer",
                "headphone", "earphone", "earbud", "speaker", "charger",
                "cable", "cover", "case", "accessory",
                # Functionality
                "works", "working", "function", "feature", "performance",
                "battery", "screen", "display", "camera", "lens", "button",
                "switch", "charging", "connection", "connects", "signal",
                "speed", "fast", "slow", "lag", "glitch", "bug", "error",
                # Food / Restaurant product
                "food", "meal", "dish", "drink", "beverage", "taste", "flavor",
                "flavour", "salty", "sweet", "spicy", "bland", "stale", "fresh",
                "hot", "cold", "warm", "raw", "burnt", "cooked", "portion",
                "serving", "menu", "order", "ordered", "cuisine",
                # Packaging & delivery (product side)
                "packaging", "package", "box", "wrap", "wrapper", "seal",
                "delivery", "arrived", "shipped", "shipping", "tracking",
                "courier", "dispatch",
                # Fabric/wearables
                "fabric", "soft", "hard", "rough", "smooth", "breathable",
                "waterproof", "comfortable", "uncomfortable", "fits", "tight",
                "loose", "stretchy",
            ],

            # ── SERVICE ─────────────────────────────────────────────────────
            # Staff, employee conduct, response, professionalism, attitude
            "service": [
                # People / roles
                "service", "staff", "employee", "waiter", "waitress", "server",
                "cashier", "receptionist", "manager", "supervisor", "worker",
                "crew", "team", "host", "hostess", "attendant", "representative",
                "rep", "agent", "operator", "technician", "driver", "delivery guy",
                "guard", "security",
                # Behaviour & attitude
                "behavior", "behaviour", "attitude", "manner", "conduct",
                "treatment", "interaction", "communication", "response",
                "rude", "rudeness", "impolite", "disrespectful", "arrogant",
                "helpful", "unhelpful", "friendly", "unfriendly", "kind",
                "unkind", "polite", "courteous", "professional", "unprofessional",
                "nice", "mean", "harsh", "aggressive", "welcoming",
                "warm", "cold", "indifferent", "hostile", "cheerful",
                # Response / support
                "support", "assist", "assistance", "help", "helped", "resolve",
                "resolved", "complaint", "complained", "respond", "responded",
                "reply", "replied", "contact", "contacted", "ignored", "ignored me",
                "listen", "listened", "care", "cared",
                # Speed / efficiency
                "waiting", "waited", "wait", "slow", "fast", "quick", "delayed",
                "delay", "late", "on time", "prompt", "efficient", "inefficient",
                # Customer experience
                "hospitality", "experience", "greeting", "greeted", "welcoming",
                "attention", "attentive", "negligent", "reckless",
                "acknowledge", "acknowledged", "remembered",
                "overcharged", "mistaken", "wrong order",
            ],

            # ── PRICE ───────────────────────────────────────────────────────
            # Cost, value for money, affordability, deals
            "price": [
                # Core terms
                "price", "cost", "fee", "charge", "rate", "fare",
                "amount", "total", "bill", "invoice", "receipt",
                # Expensive perception
                "expensive", "costly", "overpriced", "pricey", "steep",
                "ridiculous", "outrageous", "too much", "rip off", "ripoff",
                "rip-off", "daylight robbery", "highway robbery",
                # Cheap / affordable
                "cheap", "affordable", "reasonable", "fair", "budget",
                "inexpensive", "economical", "value", "worth it",
                "great deal", "good deal", "bargain", "steal",
                # Money actions
                "money", "pay", "paid", "paying", "spend", "spent",
                "spending", "afford", "afforded", "tight",
                # Discounts / promotions
                "discount", "offer", "deal", "promo", "coupon",
                "cashback", "voucher", "sale", "off", "savings",
                "refund", "return", "exchange",
                # Comparisons
                "cheaper", "pricier", "better value", "not worth",
                "overcharged", "hidden fee", "extra charge",
            ],

            # ── AMBIENCE ────────────────────────────────────────────────────
            # Environment, atmosphere, cleanliness, location, comfort
            "ambience": [
                # Core terms
                "ambience", "ambiance", "atmosphere", "environment", "vibe",
                "feel", "feeling", "mood", "aura",
                # Cleanliness
                "clean", "cleanliness", "dirty", "filthy", "messy", "hygiene",
                "hygienic", "unhygienic", "sanitised", "sanitized", "dusty",
                "stain", "stained", "smelly", "odor", "odour", "fresh",
                "bacteria", "pest", "cockroach", "insect", "rat",
                # Noise & sound
                "noise", "noisy", "loud", "quiet", "peaceful", "serene",
                "music", "sound", "volume", "disturbing", "silent",
                # Lighting & aesthetics
                "lighting", "light", "bright", "dark", "dim", "gloomy",
                "decor", "decoration", "interior", "design", "beautiful",
                "ugly", "modern", "old", "outdated", "renovated",
                # Space & comfort
                "space", "spacious", "cramped", "crowded", "packed",
                "seating", "seat", "chair", "table", "comfortable",
                "uncomfortable", "cozy", "cosy",
                # Location & access
                "location", "place", "area", "neighborhood", "zone",
                "parking", "accessible", "near", "far", "distance",
                "road", "traffic", "transport",
                # Specific venues
                "restaurant", "cafe", "hotel", "store", "shop", "mall",
                "room", "hall", "floor", "washroom", "bathroom", "toilet",
                # Temperature / comfort
                "temperature", "hot", "cold", "warm", "air conditioning",
                "ac", "fan", "ventilation", "stuffy", "freezing",
            ],
        }
        ...
        for label, score in zip(res["labels"], res["scores"]):
            ...
            if keyword_found and score > 0.30:
                # Keyword evidence → accept with low confidence bar
                aspects.append(label)
            elif not keyword_found and score > 0.60:
                # No keyword → only accept if zero-shot is very confident
                aspects.append(label)

        # Safety fallback: always return at least one aspect
        if not aspects:
            aspects = [res["labels"][0]]
