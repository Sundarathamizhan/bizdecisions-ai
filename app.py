...
def init_db():
    ...
    db_path = os.path.join(base_dir, "business_intelligence.db")
...
def load_model():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    local_dev_path = r"C:\new\my_bert_model"
    deploy_path = os.path.join(base_dir, "my_bert_model")
    
    # Use local dev path if available, otherwise fallback to deployment directory
    default_model_path = local_dev_path if os.path.exists(local_dev_path) else deploy_path
...
def _make_trend_chart(result_df: pd.DataFrame, date_col: str):
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        st.write(f"Analyzing {len(df_clean)} reviews (first 20 shown)")

        results = []
        limit = min(500, len(df_clean))  # Increased from 20 due to batch processing!
        st.write(f"Analyzing {limit} reviews...")

        # 1. Gather all inputs
        batch_inputs = [] # list of (idx, text, aspect)
        row_datas = []
        ...
        for i in range(limit):
            ...
            for aspect_name in ["product", "service", "price", "ambience"]:
                # Initialize columns so dataframe is clean
