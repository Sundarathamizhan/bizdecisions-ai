...
class Backtester:
    ...
    ----------
    ticks : int
        Total simulation length in ticks (default: 200).
    seed  : int
    ...
    def run(self):
        ...
        Call print_results() and print_advanced_metrics() afterwards.
        """
        self._plan_faults()
    ...
    # ── Results Display ───────────────────────────────────────────────────
    # FIX: removed duplicate print_results() — only one definition retained.
    ...
    def compute_roc_auc(self) -> Dict[str, dict]:
        ...
        for sid in self.ground_truth:
            scores = [
                abs(row[sid].z_score)
                if (sid in row and row[sid].z_score is not None) else 0.0
    ...
    def print_advanced_metrics(self):
        ...
        print(_c("bold", "═" * 68))
        print(_c("bold",
                  f"  {'SENSOR':<15} {'AUC':>6} {'ACC':>7} {'SPEC':>7}"
                  f" {'TP':>5} {'TN':>5} {'FP':>5} {'FN':>5}"))
        print("  " + "─" * 58)

        aucs = []
        for sid in cm:
            auc_v   = roc.get(sid, {}).get("auc", 0.0)
            aucs.append(auc_v)
            m       = cm[sid]
            auc_col = "green" if auc_v >= 0.8 else ("yellow" if auc_v >= 0.6 else "red")
            print(
                f"  {sid:<15} {_c(auc_col, f'{auc_v:.3f}'):>6}"
