...
def run_demo(args):
    ...
    print(f"       Yield: {twin_metrics.get('yield_pct', 100):.1f}% | Stress: {twin_metrics.get('stress_level', 'None')}")
    ...
    if args.ai:
        ...
        if report.actions:
            eng = advisor.decisions
            print("  Auto-Actions:\n" + eng.format_actions(
                {s: [a] for s in summary for a in report.actions}))
    ...
    try:
        while True:
            ...
            for sid, r in results.items():
                ...
                a = "!" if r["anomaly"] else " "
                line += f"  {sid[:4]}={v}{a}"
            line += f" | Yield={twin_metrics.get('yield_pct', 100):.1f}%"
            print(line)

            # Every 30 ticks, optionally ping AI
            if args.ai and tick % 30 == 0:
                summary = ml.get_summary()
                insight = advisor.diagnose(summary, score)
                print("\n  [AI DIGEST] " + insight.split("\n")[0] + "\n")

            time.sleep(args.interval)

    except KeyboardInterrupt:
        print("\n[LIVE] Stopped. Printing final summary...\n")
        Dashboard().update_bulk(ml.history)
        Dashboard().print_report()
...
def run_compare(args):
    ...
    random.seed(seed)                       # re-seed IoT simulation
    from iot_layer import SensorNetwork
    network  = SensorNetwork()
    pipeline = AdvancedMLPipeline(window=20)
    ...
    for tick in range(1, ticks + 1):
        ...
        for start, end, sid, ftype in fault_windows:
            if start <= tick <= end and sid in readings:
                ...
                r      = readings[sid]
                if ftype == "spike":
                    readings[sid].value = round(sensor.cfg.max_val * 0.95,
                                                sensor.cfg.decimals)
                elif ftype == "step":
                    readings[sid].value = round(
                        min(r.value + sensor.cfg.noise_amplitude*4, sensor.cfg.max_val),
                        sensor.cfg.decimals)
                elif ftype == "drift":
                    frac = (tick-start) / max(end-start, 1)
                    readings[sid].value = round(
                        min(r.value + frac*sensor.cfg.noise_amplitude*5, sensor.cfg.max_val),
                        sensor.cfg.decimals)
    ...
    ANSI = {"reset":"[0m","bold":"[1m","green":"[92m",
            "yellow":"[93m","red":"[91m","cyan":"[96m"}
    def C(col, t): return f"{ANSI[col]}{t}{ANSI['reset']}"

    print()
    print(C("bold", "═"*68))
    print(C("bold", "  F1 Score Comparison  (seed=42)  "
            f"Baseline (Z+IF+OLS)  vs  Advanced (DynZ+IF+OLS)"))
    print(C("bold", "═"*68))
    hdr = f"  {'SENSOR':<15} {'BASE_F1':>8} {'ADV_F1':>8} {'DELTA':>8} {'WINNER':>8}"
    print(C("bold", hdr))
    print("  "+"-"*62)
    ...
    for sid in baseline_m:
        ...
        bf, af = bm["f1"], am.get("f1", 0.0)
        delta  = af - bf
        winner = C("green", "ADVANCED") if delta > 0.01 \
                 else C("yellow", "BASELINE") if delta < -0.01 \
                 else C("cyan", "TIE     ")
